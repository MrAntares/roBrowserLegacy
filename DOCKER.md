...
Uncomment the GRF volume mounts in `docker-compose.yaml`, set `GRF_PATH`, then:
...
docker run -d --name robrowserlegacy --network host \
  ...
  ghcr.io/lenaxia/robrowserlegacy:latest-slim
```
