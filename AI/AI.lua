...
-- global variable
------------------------------------------
MyState				= IDLE_ST		-- The first state is rest.
MyEnemy				= 0				-- enemy id
MyDestX				= 0				-- destination x
MyDestY				= 0				-- destination y
MyPatrolX			= 0				-- scouting destination x
MyPatrolY			= 0				-- scouting destination y
ResCmdList			= List.new()	-- Reserved command list
MyID				= 0				-- homunculus id
MySkill				= 0				-- Homunculus skill
...
function	OnIDLE_ST ()
	...
	if (cmd ~= nil) then
		ProcessCommand (cmd)	-- Reserved command processing
		return
	end
	...
	if msg[1] == NONE_CMD then
		if rmsg[1] ~= NONE_CMD then
			if List.size(ResCmdList) < 10 then
				List.pushright (ResCmdList,rmsg) -- Save reservation order
			end
		end
	else
		List.clear (ResCmdList)	-- When a new command is input, the reserved commands are deleted.
		ProcessCommand (msg)	-- command processing
	end
