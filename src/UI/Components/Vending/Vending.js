...
define(function (require) {
	...
	Vending.onAppend = function onAppend() {
		...
		InputWindow.css({
			top: Math.min(
				Math.max(0, _preferences.inputWindow.y),
				Renderer.height - InputWindow.find('.content').height()
			),
			left: Math.min(
				Math.max(0, _preferences.inputWindow.x),
				Renderer.width - InputWindow.find('.content').width()
			)
		});
		OutputWindow.css({
			top: Math.min(
			...
			left: Math.min(
				Math.max(0, _preferences.outputWindow.x),
				Renderer.width - OutputWindow.find('.content').width()
			)
		});
		...
		return function transferItem(fromContent, toContent, isAdding, index, count) {
			...
			// Remove item
				...
				tmpItem.count =
					_type === Vending.Type.BUYING_STORE
						? _input[index].total
						: _input[index].count + _output[index].count;
