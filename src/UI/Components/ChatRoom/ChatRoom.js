...
function onMemberContextMenu(event) {
	...
	const isOwner = ChatRoom.owner === Session.Character.display.name;
	const isSelf = memberName === Session.Character.display.name;
...
ChatRoom.message = function displayMessage(message, type) {
	...
	} else if (message.indexOf(Session.Character.display.name + ' : ') === 0) {
