...
define(function (require) {
	...
	NpcStore.setType = function setType(type) {
		switch (type) {
			...
			case NpcStore.Type.VENDING_STORE:
				this.ui
					.find('.WinBuy, .WinSell, .WinCash, .WinBuyingStore, .AvailableItemsWindow, .PurchaseResult')
					.hide();
	...
	NpcStore.calculateCost = function calculateCost() {
		...
		for (i = 0; i < count; ++i) {
			if (_output[i]) {
				total +=
					(_output[i].discountprice || _output[i].overchargeprice || _output[i].price) * _output[i].count;
			}
		}
	...
	function addItem(content, item) {
		...
		if (
			...
			price = prettyZeny(
				item.price,
				_type === NpcStore.Type.VENDING_STORE || _type === NpcStore.Type.BUYING_STORE
			);
		...
		return function (fromContent, toContent, isAdding, index, count) {
			...
			if (isAdding) {
				...
				if (_type === NpcStore.Type.BARTER_MARKET) {
					...
					let inputCurrencyDiv = NpcStore.ui.find(
					...
					let outputCurrencyDiv = NpcStore.ui.find(
