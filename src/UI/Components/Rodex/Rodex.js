...
define(function (require) {
	...
	function onClickSearchTitle(e) {
		...
		Rodex.searchType = 1;
		Client.loadFile(
			DB.INTERFACE_PATH + 'basic_interface/rodexsystem/renewal/checkbox_search_off.bmp',
			function (data) {
				Rodex.ui.find('.search-sender').css('backgroundImage', 'url(' + data + ')');
			}
		);
		Client.loadFile(
			DB.INTERFACE_PATH + 'basic_interface/rodexsystem/renewal/checkbox_search_on.bmp',
			function (data) {
