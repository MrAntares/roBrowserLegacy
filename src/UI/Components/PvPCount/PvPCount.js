...
function renderRankText(text) {
	...
	const _ranking = parts[0];
	const _total = parts[1];
	const step = 28; // Compact spacing
	const slashStep = 28;

	// Calculate total width to center it
	const totalWidth = _ranking.length * step + slashStep + _total.length * step;
	let x = (RANK_W - totalWidth) >> 1;

	// 1. Render Ranking (Higher)
	let i, a;
	for (i = 0; i < _ranking.length; i++) {
		a = rankCharToAction(_ranking[i]);
		if (!isNaN(a)) {
	...
	for (i = 0; i < _total.length; i++) {
		a = rankCharToAction(_total[i]);
		if (!isNaN(a)) {
			drawActionToCanvas(_rankCtx, _rankfontAct, _rankfontSpr, a, x, RANK_Y + 6);
			x += step;
		}
