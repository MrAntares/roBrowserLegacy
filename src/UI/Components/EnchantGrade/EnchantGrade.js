...
define(function(require)
{
	...
	const scenarioMsgMapping = {
		initial: [
  		...
  		waiting: [
    		{ id: 3707, color: '#de0808' }
  		],
		cannot_proceed: [
			{ id: 3821, color: '#de0808' },
		],
		insufficient: [
			{ id: 3708, color: '#de0808' },
		],
		destroyable: [
			{ id: 3712, color: '#de0808' },
		],
		non_destroyable: [
			{ id: 3822, color: '#00ef5a' },
		]
	};
	...
	EnchantGrade.init = function init()
	{
		...
		// Some Functions
		this.ui.find('.enchant_drop_proxy')
			.on('drop', onItemDrop).on('dragover', stopPropagation);
		this.ui.find('.enchant_container')
	...
	*/
	...
	function onEnchantGradeUIUpdateMaterials(pkt)
	{
		if (pkt && pkt.materialList.length > 0) {
			...
			// ADD / REMOVE BLESSING
			EnchantGrade.ui.find('.add_material_btn').data('amount', bless.amount)
				...
				.on('mouseenter mousemove', function (e) {
					showTooltip(
						DB.getMessage(3976).replace('%d', bless.amount),
						e,
						this
					);
				})
			...
			EnchantGrade.ui.find('.remove_material_btn')
				.off('click')
				.on('click', function () {
			...
			function updateBlessing() {
				...
				EnchantGrade.ui.find('.probability')
					.text(EnchantGrade_current_success + addedChance + '%');
			
				EnchantGrade.ui.find('.BED_container .additonal_mat_amount')
					.text(totalBlessing + ' ea');
			}
	...
	function onPopulateMaterials()
	{
		...
		EnchantGrade.ui.find('.material_slot')
			...
			.on('mouseleave', function () {
				...
				if (!item || material_count < material_amount) {	// If no item or insufficient count
					insufficent = true;
				}
	...
	function onResetBlessing() {
		...
		EnchantGrade.ui.find('.probability')
			.text(EnchantGrade_current_success + '%');
			
		EnchantGrade.ui.find('.BED_container .additonal_mat_amount')
			.text(EnchantGrade_currentBlessing + ' ea');
	};
	...
	function onRemoveItem() {
		...
		EnchantGrade.ui.find('.add_material_btn')
			.off('click mouseenter mouseleave mousemove');
		...
		startBtn
			.off('click')
			.css('backgroundImage', `url(${startDisable})`);
	...
	function onBroadcastEnchantGradeResult(pkt)
	{
		if (pkt) {
			...
			let message = (DB.getMessage(messageID)).replace('%s', pkt.char_name).replace('%s', GradeMapping[pkt.grade]).replace('%s', itemName);
