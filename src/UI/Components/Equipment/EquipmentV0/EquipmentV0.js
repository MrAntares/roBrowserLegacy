...
/**
 * Check whether the payload can be dropped on EquipmentV0.
 */
...
function showEquipmentDropHighlight(data) {
	...
	const ui = EquipmentV0.ui.find(selector);
...
function clearEquipmentDropHighlight() {
	EquipmentV0.ui.find('td, .ammo').css('backgroundImage', 'none');
}
...
function onEquipmentDrop(_event, data) {
	...
	EquipmentV0.onEquipItem(item.index, getEquipmentDropLocation(item));
	return false;
}
