...
/**
 * Check whether the payload can be dropped on EquipmentV3.
 */
...
function onEquipmentDropOver(_event, data) {
	...
	const ui = EquipmentV3.ui.find(selector);
...
function clearEquipmentDropHighlight() {
	EquipmentV3.ui.find('td, .ammo').css('backgroundImage', 'none');
}
...
function onEquipmentDrop(_event, data) {
	...
	EquipmentV3.onEquipItem(item.index, getEquipmentDropLocation(item));
	return false;
}
