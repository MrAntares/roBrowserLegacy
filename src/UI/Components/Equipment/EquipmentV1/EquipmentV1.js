...
const EquipmentV1 = new GUIComponent('EquipmentV1', cssText);

EquipmentV1.render = () => htmlText;
	...
	if (UIVersionManager.getEquipmentVersion() > 0) {
		...
		if (lvlupEl) {
			...
			_btnLevelUp.addEventListener('click', () => {
				...
				if (EquipmentV1._host.style.display !== 'none') {
	...
	if (this._host.style.display === 'none') {
		...
		if (UIVersionManager.getEquipmentVersion() > 0) {
			...
			if (_preferences.stats) {
				WinStats.getUI().embed(EquipmentV1._host);
			}
		}
		this.focus();
	} else {
