	...
	if (this.ui.is(':visible')) {
		...
		if (UIVersionManager.getEquipmentVersion() > 0) {
			...
			if (_preferences.stats) {
				WinStats.getUI().embed(EquipmentV1.ui[0]);
			}
		}
		this.focus();
	} else {
...
/**
 * Check whether the payload can be dropped on EquipmentV1.
 */
...
function showEquipmentDropHighlight(data) {
	...
	const ui = EquipmentV1.ui.find(selector);
...
function clearEquipmentDropHighlight() {
	EquipmentV1.ui.find('td, .ammo').css('backgroundImage', 'none');
}
...
function onEquipmentDrop(_event, data) {
	...
	EquipmentV1.onEquipItem(item.index, getEquipmentDropLocation(item));
	return false;
}
