	...
	if (this.ui.is(':visible')) {
		...
		if (UIVersionManager.getEquipmentVersion() > 0) {
			...
			if (_preferences.stats) {
				WinStats.getUI().embed(EquipmentV2.ui[0]);
			}
		}
		this.focus();
	} else {
...
/**
 * Check whether the payload can be dropped on EquipmentV2.
 */
...
function showEquipmentDropHighlight(data) {
	...
	const ui = EquipmentV2.ui.find(selector);
...
function clearEquipmentDropHighlight() {
	EquipmentV2.ui.find('td, .ammo').css('backgroundImage', 'none');
}
...
function onEquipmentDrop(_event, data) {
	...
	EquipmentV2.onEquipItem(item.index, getEquipmentDropLocation(item));
	return false;
}
