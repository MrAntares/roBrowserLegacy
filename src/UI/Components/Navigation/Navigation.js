...
function getPlayerPosition() {
	...
	const currentX = Math.ceil(Session.Character.position[0]);
	const currentY = Math.ceil(Session.Character.position[1]);
...
Navigation.renderCanvas = function renderCanvas(tick) {
	...
	if (_arrow.complete && _arrow.width) {
		...
		ctx.rotate(((Session.Character.direction + 4) * 45 * Math.PI) / 180);
