...
define(function(require)
{
	...
	InventoryV2.onAppend = function OnAppend()
	{
		...
		Client.loadFile( DB.INTERFACE_PATH + 'basic_interface/tab_itm_0'+ (_preferences.tab+1) +'.bmp', function(data){
			InventoryV2.ui.find('.tabs').css('backgroundImage', 'url("' + data + '")');
		});
	 ...
	 * @param {number} height
	 */
	InventoryV2.resize = function Resize( width, height )
	{
	...
	InventoryV2.addItemtoSwitch = function(index) {
		...
		/** Need to add here as well because Inventory packets were sent first,
		 *  and items are already listed before Swap List were sent
		 */
		var tab = InventoryV2.TAB.EQUIP;
