...
define(function (require) {
	...
	DB.prepareAIFiles = async function prepareAIFiles() {
		...
		function addAIctx() {
			...
			var Renderer = getModule('Renderer/Renderer');
			var AIDriver = getModule('Core/AIDriver');
			...
			ctx.GetV = function (V_, id) {
					...
					if (skillId && skilLevel) {
						...
						if (
							...
							let distance = Math.sqrt(
								Math.pow(homun?.position[0] - entity?.position[0], 2) +
									Math.pow(homun?.position[1] - entity?.position[1], 2)
							);
				...
				case 14: // V_SKILLATTACKRANGE_LEVEL
					...
					range = SkillInfo[id].AttackRange[skilLevel - 1] + 1 || homun.attack_range || 1;
					return range;
				*/
					default:
				...
				Client.loadFile(
					...
					function (file) {
						try {
							...
							// Coletar promessas dos dofiles aninhados
							...
							// Aguardar todos os dofiles aninhados antes de resolver
					...
					Client.loadFile(
						...
						function (file) {
							try {
								...
								// Aguardar todos os dofiles aninhados
								Promise.all(nestedPromises)
									.then(() => {
								...
								console.error('[prepareAIFiles1] : ', error);
								reject(error);
							}
						},
						reject
		...
		addAIctx();
		// Aguardar todos os arquivos carregarem antes de executar
		try {
			await Promise.all(loadPromises);
			console.log('All AI files loaded, executing...');
			// Executar todos os arquivos carregados
			for (const key in loadedFiles) {
				lua.doFileSync(loadedFiles[key]);
			}
		} catch (error) {
