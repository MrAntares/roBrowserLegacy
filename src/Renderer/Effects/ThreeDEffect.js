	...
	if (this.actRessource && this.spriteRessource) {
		...
		const actions =
			this.actRessource.actions[
				(entity.action * 8 + ((Camera.direction + entity.direction + 8) % 8)) % this.actRessource.actions.length
			];
		let animations;
		const delay = this.sprDelay || actions.delay;
		if (this.playSprite) {
			animations = actions.animations[Math.floor((tick - this.startTick) / delay) % actions.animations.length];
		} else {
