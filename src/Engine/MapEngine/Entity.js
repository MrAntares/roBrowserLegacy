...
function onEntitySpam(pkt) {
	...
	if (entity.GUID) {
		Guild.requestGuildEmblem(entity.GUID, entity.GEmblemVer, (image, gif) => {
			...
			if (Session.mapState.isSiege && entity.GUID !== Session.Character.GUID) {
...
function onEntityVanish(pkt) {
	...
	if (entity) {
		if (entity.objecttype === Entity.TYPE_PC && pkt.GID === Session.Character.GID) {
	...
	// Show escape menu
	if (pkt.GID === Session.Character.GID && pkt.type === 1) {
...
function onEntityAction(pkt) {
	...
	switch (pkt.action) {
		...
		case 13: {
			...
			// Talk sometime
			if (
				srcEntity.GID === Session.Character.GID &&
			...
			if (
				...
				if (entity === Session.Character) {
					...
					if (PACKETVER.value >= 20200520) {
						...
						BasicInfo.getUI().update('bexp', Session.Character.base_exp, BasicInfo.getUI().base_exp_next);
...
function onEntityCastSkill(pkt) {
	...
	Session.Character.isCastingSkill = true;
	Session.Character.lastSKID = pkt.SKID;
	...
	if (pkt.SKID == SkillId.KN_AUTOCOUNTER) {
		...
		if (srcEntity === Session.Character) {
	...
	if (entity) {
		...
		if (entity === Session.Character) {
			...
			if (Session.underAutoCounter) {
				if (Session.Character.life.hp > 0) {
					const EF_Init_Par = {
...
function onEntityStatusChange(pkt) {
	...
	switch (pkt.index) {
		...
		case StatusConst.HALLUCINATIONWALK:
			...
			if (entity === Session.Character && [StatusConst.SOULLINK, StatusConst.SKE].includes(pkt.index)) {
		...
		case StatusConst.POSTDELAY:
			if (pkt.RemainMS && entity == Session.Character) {
