...
define(function (require) {
	...
	function onMessage(pkt) {
		if (pkt.color) {
			ChatBox.addText(
				DB.getMessage(pkt.msg),
				ChatBox.TYPE.PUBLIC,
				ChatBox.FILTER.PUBLIC_LOG,
				uint32ToRGB(pkt.color)
			);
		} else {
	...
	function onRatesInfo(pkt) {
		...
		message +=
			DB.getMessage(1933) + formatRate(DB.getMessage(3032), pkt.total_exp, pkt.info, 'exp', serverName) + '\n';
		message +=
			DB.getMessage(1934) + formatRate(DB.getMessage(3032), pkt.total_drop, pkt.info, 'drop', serverName) + '\n';
		message +=
			DB.getMessage(1935) +
			formatRate(DB.getMessage(3032), pkt.total_death, pkt.info, 'death', serverName) +
			'\n';
