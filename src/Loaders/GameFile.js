...
define([
  ...
  GRF.prototype.load = function Load(file) {
    ...
    if (
      header.file_table_offset + GRF.struct_header.size > file.size ||
      header.file_table_offset < 0
    ) {
      throw new Error(
        "GRF::load() - Can't jump to table list (" +
    ...
    // Load Table Info
    buffer = reader.load(
      header.file_table_offset + GRF.struct_header.size,
      GRF.struct_table.size
    );
    fp = new BinaryReader(buffer);
  ...
  GRF.prototype.search = (function searchClosure() {
    ...
    return function search(filename) {
      ...
      if (
        ...
        entries[range[1]].filename === filename
      ) {
        return range[1];
      }
