...
class GameFileDecrypt {
	static _mask = new Uint8Array([0x80, 0x40, 0x20, 0x10, 0x08, 0x04, 0x02, 0x01]);
	static _tmp = new Uint8Array(8);
	static _tmp2 = new Uint8Array(8);
	static _clean = new Uint8Array(8);
	 ...
	 * @param {number} index
	 */
	static initialPermutation(src, index) {
		let i, j;

		for (i = 0; i < 64; ++i) {
			j = this._initialPermutationTable[i] - 1;
			if (src[index + ((j >> 3) & 7)] & this._mask[j & 7]) {
				this._tmp[(i >> 3) & 7] |= this._mask[i & 7];
			}
		}

		src.set(this._tmp, index);
		this._tmp.set(this._clean);
	}

	static _initialPermutationTable = new Uint8Array([
	 ...
	 * @param {number} index
	 */
	static finalPermutation(src, index) {
		let i, j;

		for (i = 0; i < 64; ++i) {
			j = this._finalPermutationTable[i] - 1;
			if (src[index + ((j >> 3) & 7)] & this._mask[j & 7]) {
				this._tmp[(i >> 3) & 7] |= this._mask[i & 7];
			}
		}
	...
	static _finalPermutationTable = new Uint8Array([
	 ...
	 * @param {number} index
	 */
	static transposition(src, index) {
		let i, j;

		for (i = 0; i < 32; ++i) {
			j = this._transpositionTable[i] - 1;
			if (src[index + (j >> 3)] & this._mask[j & 7]) {
				this._tmp[(i >> 3) + 4] |= this._mask[i & 7];
			}
		}
	...
	static _transpositionTable = new Uint8Array([
	 ...
	 * @param {number} index
	 */
	static expansion(src, index) {
		this._tmp[0] = ((src[index + 7] << 5) | (src[index + 4] >> 3)) & 0x3f; // ..0 vutsr
		this._tmp[1] = ((src[index + 4] << 1) | (src[index + 5] >> 7)) & 0x3f; // ..srqpo n
		this._tmp[2] = ((src[index + 4] << 5) | (src[index + 5] >> 3)) & 0x3f; // ..o nmlkj
		this._tmp[3] = ((src[index + 5] << 1) | (src[index + 6] >> 7)) & 0x3f; // ..kjihg f
		this._tmp[4] = ((src[index + 5] << 5) | (src[index + 6] >> 3)) & 0x3f; // ..g fedcb
		this._tmp[5] = ((src[index + 6] << 1) | (src[index + 7] >> 7)) & 0x3f; // ..cba98 7
		this._tmp[6] = ((src[index + 6] << 5) | (src[index + 7] >> 3)) & 0x3f; // ..8 76543
		this._tmp[7] = ((src[index + 7] << 1) | (src[index + 4] >> 7)) & 0x3f; // ..43210 v
	 ...
	 * @param {number} index
	 */
	static substitutionBox(src, index) {
		let i;

		for (i = 0; i < 4; ++i) {
			this._tmp[i] =
				(this._substitutionBoxTable[i][src[i * 2 + 0 + index]] & 0xf0) |
				(this._substitutionBoxTable[i][src[i * 2 + 1 + index]] & 0x0f);
		}
	 ...
	 * @param {number} index
	 */
	static roundFunction(src, index) {
		for (let i = 0; i < 8; i++) {
			this._tmp2[i] = src[index + i];
		}

		this.expansion(this._tmp2, 0);
		this.substitutionBox(this._tmp2, 0);
		this.transposition(this._tmp2, 0);

		src[index + 0] ^= this._tmp2[4];
		src[index + 1] ^= this._tmp2[5];
		src[index + 2] ^= this._tmp2[6];
		src[index + 3] ^= this._tmp2[7];
	}
	 ...
	 * @param {number} index
	 */
	static decryptBlock(src, index) {
		this.initialPermutation(src, index);
		this.roundFunction(src, index);
		this.finalPermutation(src, index);
	}
	 ...
	 * @param {number} index
	 */
	static shuffleDec(src, index) {
		this._tmp[0] = src[index + 3];
		this._tmp[1] = src[index + 4];
		this._tmp[2] = src[index + 6];
		this._tmp[3] = src[index + 0];
		this._tmp[4] = src[index + 1];
		this._tmp[5] = src[index + 2];
		this._tmp[6] = src[index + 5];
		this._tmp[7] = this._shuffleDecTable[src[index + 7]];
		src.set(this._tmp, index);
		this._tmp.set(this._clean);
	}
