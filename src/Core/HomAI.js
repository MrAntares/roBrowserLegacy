define(['Renderer/EntityManager', 'Renderer/Renderer', 'Vendors/fengari-web', 'Renderer/Entity/Entity'], function (EntityManager, Renderer, fengari, Entity) {
    ...
    HomAI.init = function init() {
        ...
        var code = `
            ...
            if (ai_main) then
                ai_main()
                js.global.console:log("%c[homAI] %cAI initialized.", "color:#DD0078", "color:inherit")
            else
                js.global.console:warn("%c[homAI] %cCould not load AI: " .. ai_error, "color:#DD0078", "color:inherit")
            end
    ...
    HomAI.setmsg = function setmsg(homId, str) {
        msg[homId] = str;
    }
    ...
    window.GetActors = function () {
        ...
        // aggressive logic
        if (res.length > 3) {
            if (localStorage.getItem('HOM_AGGRESSIVE') == 1) {
                res.forEach((item) => {
                    if (item != 0 && item != Session.AID && item != Session.homunId) {
                        ...
                        if (entity && (entity.objecttype === Entity.TYPE_MOB || entity.objecttype === Entity.TYPE_NPC_ABR || entity.objecttype === Entity.TYPE_NPC_BIONIC)) {
                            if (status == 0) { //idle = 0
                                // attak
                                HomAI.setmsg(Session.homunId, '3,'+ item);
                            }
                        }
                    }
                })
            } else {
                HomAI.setmsg(Session.homunId, status);
            }
        }
        return res;
    }
    ...
    HomAI.exec = function exec(code) {
        try {
            fengari.load(code)();
        } catch (e) {
            console.error('%c[homAI] %cAI Error: ', "color:#DD0078", "color:inherit", e);
        }
    }

    HomAI.reset = function reset(){
        this.init();
    }
